/**
 * Hero Rotation - Webflow-compatible vanilla JS module
 * Single-file particle logo animation with continuous rotation
 * Three.js only, no GSAP
 */
...
/**
 * Initialize the hero rotation animation
 ...
 * @param {number} [options.rotationSpeed=0.0025] - Y-axis rotation speed per frame
 * @returns {{ destroy: () => void }}
 */
function initHeroRotation(options = {}) {
    const {
    ...
    if (!canvas) {
        console.error('HeroRotation: No canvas found. Provide canvas, container, or add a canvas.webgl element.')
        return { destroy: () => {} }
    }
    ...
    function tick() {
        ...
        particles.points.rotation.y += rotationSpeed
    ...
    loadImagePixels(imageUrl)
        ...
        .catch((err) => {
            console.warn('HeroRotation: Could not load image, using fallback grid:', err.message)
...
export default initHeroRotation
